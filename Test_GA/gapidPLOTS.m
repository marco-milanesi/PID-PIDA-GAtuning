...
fval_PID
%% PI-D
options_PI_D = optimoptions(@ga,'PopulationSize',popsize,'MaxGenerations',MaxGenerations,'InitialPopulation',population_PI_D,'OutputFcn',@myfun_pid);
[x_PI_D,fval_PI_D,exitflag,output,population_PI_D,scores] = ga(@(K)dpitest(G,dt,K),3,-eye(3),zeros(3,1),[],[],[],[],[],options_PID);
...
[x_I_PD,fval_I_PD,exitflag,output,population_I_PD,scores] = ga(@(K)ipdtest_gapid(G,dt,K),3,-eye(3),zeros(3,1),[],[],[],[],[],options_PID);
